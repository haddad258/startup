...
const ImagesConfigsC = ({ refresh, selectedImagesConfigs, type }) => {
    ...
    const handleAddOrUpdate = async () => {
        ...
        if (selectedImagesConfigs?.id) {
            formData.append('selectedImagesConfigs', selectedImagesConfigs.id);
        }
        if (selectedImagesConfigs?.id) {
            formData.append('name', selectedImagesConfigs.name||selectedImagesConfigs.username);
        }
        if (file) {
            formData.append('file', file);
        }
        if (selectedImagesConfigs?.id) {
            formData.append('id', selectedImagesConfigs.id||selectedImagesConfigs.username);
        }
        

        try {
            const result = await settingsPublications.addImagesPublicationConfigs(formData, {
                headers: {
    ...
    return (
        <>
            <CButton color={"success"} onClick={() => newDataImagesConfigs()}>
                <CIcon icon={cilImage} />
            </CButton>
            ...
            >
                <CModalHeader onClose={() => setVisible(false)}>
                    <CModalTitle id="LiveDemoExampleLabel">{i18n.t('importImageTitle')} Publications</CModalTitle>
                </CModalHeader>
                <CModalBody>
                    <CForm
...
ImagesConfigsC.propTypes = {
    refresh: PropTypes.func.isRequired,
    selectedImagesConfigs: PropTypes.object,
    type: PropTypes.string,
};

export default ImagesConfigsC;
