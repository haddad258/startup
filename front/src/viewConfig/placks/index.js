
  import React, { useEffect, useState } from 'react';
  import {
      CRow,
      CCol,
      CCard,
      CCardHeader,
      CCardBody,
  } from '@coreui/react';
  import { settingsPlacks } from 'src/services/SupperSettings/index';
  import GenericTable from 'src/components/Generic.Table';
  import PlacksC from './Plack'
  import i18n from 'src/i18n';
  const Placks = () => {
      const [List, setList] = useState([]);
  
      const fetchPlacks = async () => {
          try {
              const list = await settingsPlacks.getPlacks();
              if (list) {
                  setList(list?.data);
              }
          } catch (error) {
              console.error('Error fetching admin list:', error);
          }
      };
      useEffect(() => {
          fetchPlacks();
      }, []);
  
      const columns = [
          { label: '#', field: 'index' },
          { label: i18n.t('nameInputLabel'), field: 'name' },
          { label: i18n.t('descriptionInputLabel'), field: 'description' },
          { label: i18n.t('quantityLabel'), field: 'quantity' },
          { label: i18n.t('unitPriceLabel'), field: 'unit_price' },
          { label: i18n.t('providerLabel'), field: 'provider' },
          
          {
              label:  i18n.t('actionLabel'),
              field: 'actions',
              render: (item) => <PlacksC refresh={()=>fetchPlacks()} selectedPlacks={item} />,
          },
      ];
  
      return (
          <CCard className="mb-4">
              <CCardHeader>
                  <CRow className="align-items-center">
                      <CCol md="6" xs="12">
                          <strong>   { i18n.t('PlacksTableTitle')}</strong>
                      </CCol>
                      <CCol md="6" xs="12" className="text-md-end mt-md-0 mt-3">
                          <PlacksC refresh={()=>fetchPlacks()} />
                      </CCol>
                  </CRow>
              </CCardHeader>
              <CCardBody>
                  <CRow>
                      <CCol xs="12">
                          <CCard className="mb-4">
                              <CCardHeader>
                          <strong>{ i18n.t('PlacksList')}</strong>
                              </CCardHeader>
                              <CCardBody>
                                      <GenericTable columns={columns} data={List} />
                              </CCardBody>
                          </CCard>
                      </CCol>
                  </CRow>
              </CCardBody>
          </CCard>
      );
  };
  
  export default Placks;
  
  