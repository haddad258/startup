  ...
  const Items = () => {
      ...
      const fetchItems = async () => {
          try {
              const list = await settingsConfigApps.getConfigApps("entity/items");
              if (list) {
                  setList(list?.data);
              }
          } catch (error) {
      ...
      const columns = [
          ...
          
          {
              ...
              field: 'actions',
              render: (item) => <ItemsC refresh={()=>fetchItems()} selectedItems={item} />,
          },
      ];
          ...
          <CCard className="mb-4">
              <CCardHeader>
                  <CRow className="align-items-center">
                      <CCol md="6" xs="12">
                          <strong>   { i18n.t('ItemsTableTitle')}</strong>
                      </CCol>
                      <CCol md="6" xs="12" className="text-md-end mt-md-0 mt-3">
                          <ItemsC refresh={()=>fetchItems()} />
                      </CCol>
                  </CRow>
              </CCardHeader>
              <CCardBody>
                  <CRow>
                      <CCol xs="12">
                          <CCard className="mb-4">
                              <CCardHeader>
                          <strong>{ i18n.t('ItemsList')}</strong>
                              </CCardHeader>
                              <CCardBody>
