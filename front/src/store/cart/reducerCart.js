...
const addToCart = (state, payload) => {
  ...
  const newQuantity = existingProduct
    ? existingProduct.quantity + quantity // Add to existing quantity
    : quantity; // Initialize with new quantity if not in cart
  const updatedProducts = {
    ...state.products,
    [id]: { product, quantity: newQuantity },
  };
  const updatedTotal = Object.values(updatedProducts).reduce(
